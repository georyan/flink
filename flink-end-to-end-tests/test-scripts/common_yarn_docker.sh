...
function start_hadoop_cluster() {
    ...
    while [ "$nm_running" -lt "2" ]; do
        ...
        if [ $time_diff -ge $MAX_RETRY_SECONDS ]; then
            echo "TIMEOUT. docker logs master:"
            docker logs master
            echo "TIMEOUT. docker logs slave1:"
            docker logs slave1
            echo "TIMEOUT. docker logs slave2:"
            docker logs slave2
            echo "TIMEOUT. docker logs kdc:"
            docker logs kdc
            echo "TIMEOUT. docker ps"
            docker ps
            echo "TIMEOUT. yarn node list"
            docker exec master bash -c "yarn node -list"
            return 1
        else
